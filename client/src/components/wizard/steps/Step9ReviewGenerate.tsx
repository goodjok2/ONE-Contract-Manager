...
export const Step9ReviewGenerate: React.FC = () => {
  ...
  const generateContractContent = (contractType: string) => {
    const date = formatDate(projectData.effectiveDate);
    const clientName = projectData.clientLegalName || 'Client';
    const projectName = projectData.projectName || 'Project';
    const siteAddress = `${projectData.siteAddress}, ${projectData.siteCity}, ${projectData.siteState} ${projectData.siteZip}`;
...
PARTIES:
Client: ${clientName}
Entity Type: ${projectData.clientEntityType}
State: ${projectData.clientState}
Signer: ${projectData.clientSignerName}

Child LLC: ${projectData.childLlcName}
State of Formation: ${projectData.childLlcState}
...
Total Contract Price: ${formatCurrency(projectData.totalPreliminaryContractPrice)}

PAYMENT MILESTONES:
Milestone 1: ${projectData.milestone1Percent}%
Milestone 2: ${projectData.milestone2Percent}%
Milestone 3: ${projectData.milestone3Percent}%
Milestone 4: ${projectData.milestone4Percent}%
Milestone 5: ${projectData.milestone5Percent}%
Retainage: ${projectData.retainagePercent}%

WARRANTY TERMS:
Fit & Finish: ${projectData.warrantyFitFinishMonths} months
Building Envelope: ${projectData.warrantyBuildingEnvelopeMonths} months
Structural: ${projectData.warrantyStructuralMonths} months

LEGAL JURISDICTION:
State: ${projectData.projectState}
County: ${projectData.projectCounty}
Federal District: ${projectData.projectFederalDistrict}
Arbitration Provider: ${projectData.arbitrationProvider}

---
Generated by Dvele Contract Platform
`;
    } else if (contractType === 'MANUFACTURING') {
      return `MANUFACTURING SUBCONTRACT
...
SCOPE:
Factory production of modular home units for delivery to:
${siteAddress}

MANUFACTURING PAYMENTS:
Design Payment: ${formatCurrency(projectData.manufacturingDesignPayment)}
Production Start: ${formatCurrency(projectData.manufacturingProductionStart)}
Production Complete: ${formatCurrency(projectData.manufacturingProductionComplete)}
Delivery Ready: ${formatCurrency(projectData.manufacturingDeliveryReady)}

TIMELINE:
Design Phase: ${projectData.designPhaseDays} days
Manufacturing Duration: ${projectData.manufacturingDurationDays} days

MANUFACTURER: ${projectData.manufacturerName}

---
Generated by Dvele Contract Platform
`;
    } else {
      return `ONSITE SUBCONTRACT
...
SCOPE:
Site preparation, foundation, utility connections, and modular home installation.

CONTRACTOR:
Name: ${projectData.contractorName}
License: ${projectData.contractorLicense}
Address: ${projectData.contractorAddress}
Insurance: ${projectData.contractorInsurance}

TIMELINE:
On-Site Duration: ${projectData.onsiteDurationDays} days

DELIVERY & INSTALLATION:
${formatCurrency(projectData.deliveryInstallationPrice)}
...
`;
  ...
  const handleDownload = (contractType: string, contractName: string) => {
    const content = generateContractContent(contractType);
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `${projectData.projectNumber || 'contract'}_${contractName.replace(/\s+/g, '_')}.txt`;
  ...
  const handleDownloadAll = () => {
    contracts.forEach((contract) => {
      setTimeout(() => {
        handleDownload(contract.type, contract.name);
      }, 100 * contracts.indexOf(contract));
    });
  };
